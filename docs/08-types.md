...
```typescript
interface Boy {
  /** A unique identifier, generated by Supabase or a temporary one for offline creation. */
  id?: string;
...
```typescript
type UserRole = 'admin' | 'captain' | 'officer' | 'pending';
```
...
Extends the Supabase `User` type to include the custom `UserRole`.
