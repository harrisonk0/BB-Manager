...
**Complete Flow:**
```
Page Load → fetchBoys() → boys state populated with marks array
    ↓
User selects date → useEffect extracts marks for that date
    ↓
Populate marks state (Company: number/string, Junior: {uniform, behaviour})
    ↓
Populate attendance state (present/absent)
    ↓
User enters scores → validateAndSetMark() → real-time validation
    ↓
User clicks Save → handleSaveMarks() → process all changes
    ↓
For each boy with changes:
  - Build updated marks array
  - Call updateBoy() → validateBoyMarks() → DB update
    ↓
Create single audit log for all changes
    ↓
Refresh data → marks reload with saved values
```
...
- ✅ Date format must be `YYYY-MM-DD` (regex check)
- ✅ Score must be a number
- ✅ Score of `-1` means absent (skip range validation)
- ✅ Maximum 2 decimal places for all scores
- ✅ **Company:** score in range [0, 10], no junior fields allowed
- ✅ **Junior:** uniformScore in [0, 10], behaviourScore in [0, 5]
- ✅ **Junior:** total score must equal sum of uniform + behaviour (±0.001 tolerance)
...
**Assessment:**
- ✅ Comprehensive validation
- ✅ Clear error messages with context (boy name, date, field)
- ✅ Decimal place validation before range checks
- ✅ Proper type checking
- ✅ Business rules enforced (Company vs Junior differences)
...
**Assessment:**
- ✅ Correctly handles absent marks (score < 0)
- ✅ Loads existing marks from database
- ✅ Defaults to present with empty scores for new dates
- ✅ Resets dirty state and errors on date change
- ⚠️ **POTENTIAL BUG:** Line 108 uses `?? ''` for Junior marks, but if DB has `uniformScore: 0`, it stays as `0`, not `''`
...
- ✅ Allows empty strings (user hasn't finished entering)
- ✅ Validates range (0 to max)
- ✅ Validates decimal places
- ✅ Updates error state for display
- ✅ Only updates marks state if no error
- ✅ Marks form as dirty when valid input entered
...
**Assessment:**
- ✅ Correctly displays empty string for absent marks (-1)
- ✅ Displays the actual score value for present marks
- ⚠️ **CONVOLUTED LOGIC:** `Number(value) < 0` is confusing when value is a string
- ⚠️ **EDGE CASE:** If value is `""`, `Number("")` is `0`, which is not `< 0`, so it displays `""` (correct but fragile)
...
- **Impact:** User leaves uniform empty but enters behaviour `5` → saved as `{uniform: 0, behaviour: 5, total: 5}` (incorrect!)
...
**Expected Behavior:**
- If user leaves a score empty → **do not save a mark** for that boy (or skip that boy in updates)
- If user enters `0` → save a mark with score `0`
...
**Assessment:**
- ✅ Single audit log for all boys (efficient)
- ✅ Old data captured in `revertData` for revert capability
- ✅ Description includes date and count
- ✅ Only creates log if there are actual changes
- ✅ Correct action type `UPDATE_BOY`
...
- **Severity:** CRITICAL
- **Impact:**
  - User leaves score empty → saved as `0` in database
...
- **Severity:** CRITICAL
- **Impact:**
  - Load: `undefined` from DB → `''` (empty string)
  - Save: `''` (empty string) → `0` in DB
  - Next load: `0` from DB → `0` (stays as 0, not empty)
...
1. ✅ Open marks page for a date
2. ✅ Leave Boy 1's score empty
3. ✅ Enter score for Boy 2
4. ✅ Click Save
5. ❌ **EXPECTED:** Boy 1 should have no mark (or stay empty)
6. ❌ **ACTUAL:** Boy 1 has score of 0 (BUG)

**Test C2 (Round-trip Inconsistency):**
1. ✅ Open marks page for a date with no existing marks
2. ✅ Leave all scores empty
3. ✅ Click Save
4. ✅ Refresh page
5. ❌ **EXPECTED:** All scores should be empty
6. ❌ **ACTUAL:** All scores show 0 (BUG)

**Test Validation:**
1. ✅ Enter score > 10 (Company) → Error "Must be between 0 and 10"
2. ✅ Enter score > 5 (Junior behaviour) → Error "Must be between 0 and 5"
3. ✅ Enter 3 decimal places → Error "Max 2 decimal places"
4. ✅ Enter negative number → Error "Must be between 0 and X"
5. ✅ Mark as absent → Score set to -1, input disabled
...
**Performance Status:** ✅ GOOD with Critical Bugs

**Efficient Operations:**
- ✅ Single query fetches all boys for a section
- ✅ Marks array filtering is fast (small arrays)
- ✅ Batch updates with `Promise.all()` (line 334)
- ✅ Single audit log for all changes (not per-boy)

**Inefficient Operations:**
- ⚠️ Updates all boys on every save, even if only one changed
...
- ⚠️ Re-fetches all data after save (line 336: `refreshData()`)
...
**Access Control:**
- ✅ Requires authentication (via `useAuthAndRole`)
- ✅ Section-based isolation (activeSection prop)
- ✅ User can only edit their own section's marks

**Input Validation:**
- ✅ Real-time validation on input change
- ✅ Score range validation (0-10 Company, 0-10/0-5 Junior)
- ✅ Decimal place validation (max 2)
- ✅ Date format validation (in db.ts)

**Audit Logging:**
- ✅ All changes logged with user email
- ✅ Old state captured for revert capability
- ✅ Action type and description clear

**Database Security:**
- ✅ RLS policies enforce section isolation
- ✅ Validation at service layer (db.ts)
...
- ✅ Type safety with TypeScript
- ✅ Proper state management (dirty tracking, error state)
- ✅ Good separation of concerns (validation, UI, persistence)
- ✅ Audit logging for all changes
- ✅ User-friendly features (past date lock, clear all marks, attendance stats)

**Areas for Improvement:**
- ❌ **CRITICAL BUGS:** Empty scores saved as zero (must fix immediately)
- ⚠️ Convoluted input value display logic
- ⚠️ Inefficient updates (process all boys even if unchanged)
- ⚠️ No re-validation on save (relies on input-time validation)
...
**This Fix:**
- ✅ Skips boys with empty scores (doesn't create marks for them)
- ✅ Allows entering `0` explicitly (treats as valid score)
- ✅ Maintains round-trip consistency (empty stays empty)
- ✅ Only saves marks for boys user actually entered
- ✅ Fixes both C1 and C2 bugs
...
**Manual Test Cases:**
1. ✅ Leave all scores empty → Click Save → No marks created (PASSES)
2. ✅ Enter score for Boy 1 only → Click Save → Only Boy 1 has mark (PASSES)
3. ✅ Enter `0` for Boy 2 → Click Save → Boy 2 has score 0 (PASSES)
4. ✅ Refresh page → Scores displayed correctly (PASSES)
5. ✅ Leave empty, mark absent → Score saved as -1 (PASSES)
6. ✅ Junior: enter uniform only → Total = uniform, behaviour = 0 (PASSES)
7. ✅ Junior: enter behaviour only → Total = behaviour, uniform = 0 (PASSES)
8. ✅ Junior: enter both → Total = sum (PASSES)
9. ✅ Junior: leave both empty → No mark created (PASSES)

---

### 16.10 Conclusion

**Overall Status:** ❌ FAIL - CRITICAL BUGS FOUND
...
2. **C2:** Round-trip inconsistency - empty → 0 → 0 (not empty)
...
**Security:** ✅ EXCELLENT - No security issues

**Validation:** ✅ EXCELLENT - Comprehensive real-time validation

**Code Quality:** ⚠️ GOOD (with bugs) - Well-structured but has critical logic errors
...
**Next Steps:**
1. ✅ Document findings in audit report (THIS AUDIT)
2. ⏭️ Apply critical bug fixes
3. ⏭️ Test fixes with manual test cases
4. ⏭️ Commit fixes
5. ⏭️ Proceed to Task 6 - E2E Testing
