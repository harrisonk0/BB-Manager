...
- `npm run check:db-contract` reads `.env` and `.env.local`, signs in with the test user, resolves `current_app_role()`, and confirms the seeded `settings` rows for `company` and `junior` are readable through the published client credentials.
