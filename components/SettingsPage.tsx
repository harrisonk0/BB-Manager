...
const SettingsPage: React.FC<SettingsPageProps> = ({ activeSection, currentSettings, onSettingsSaved, showToast, userRole }) => {
  ...
  const [selectedInitialRoleForInvite, setSelectedInitialRoleForInvite] = useState<UserRole>('officer'); // New state for initial role
  ...
  const handleGenerateCode = async () => {
    ...
    try {
      ...
      const newInviteCode: Omit<InviteCode, 'generatedAt' | 'id'> = {
        generatedBy: userEmail,
        isUsed: false,
        section: activeSection,
        initialRole: selectedInitialRoleForInvite, // Pass the selected initial role
      };
      ...
      await createAuditLog({
        ...
        description: `Generated new invite code: ${createdCode.id} (Initial Role: ${USER_ROLE_DISPLAY_NAMES[createdCode.initialRole || 'officer']}).`,
    ...
    <div className="space-y-6">
      ...
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        {canManageInviteCodes && (
            ...
            <div className="mb-4">
                <label htmlFor="initial-role-select" className="block text-sm font-medium text-slate-700 mb-1">
                    Assign Initial Role
                </label>
                <select
                    id="initial-role-select"
                    value={selectedInitialRoleForInvite}
                    onChange={(e) => setSelectedInitialRoleForInvite(e.target.value as UserRole)}
                    className={`mt-1 block w-full sm:w-auto px-3 py-2 bg-white border border-slate-300 rounded-md shadow-sm focus:outline-none sm:text-sm ${accentRing}`}
                    disabled={isGeneratingCode}
                >
                    {/* Only allow generating codes for roles the current user can assign */}
                    {Object.entries(USER_ROLE_DISPLAY_NAMES)
                        .filter(([roleValue]) => {
                            if (userRole === 'admin') return true;
                            if (userRole === 'captain') return roleValue === 'officer' || roleValue === 'captain';
                            return false; // Officers cannot generate codes
                        })
                        .map(([roleValue, displayName]) => (
                            <option key={roleValue} value={roleValue}>{displayName}</option>
                        ))}
                </select>
                <p className="mt-1 text-xs text-slate-500">This role will be automatically assigned to the user who signs up with this code.</p>
            </div>
            ...
            {loadingInviteCodes ? (
                ...
                {inviteCodes.map(code => (
                    ...
                    <div className="flex-1">
                        ...
                        {code.initialRole && ` (Initial Role: ${USER_ROLE_DISPLAY_NAMES[code.initialRole]})`}
