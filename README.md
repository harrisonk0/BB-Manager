...
- `npm run test:e2e` reads `.env` and `.env.local` and runs Playwright against a real Supabase-backed environment. It requires `E2E_TEST_EMAIL` and `E2E_TEST_PASSWORD`.
  - `tests/e2e/isolated-app-flows.e2e.ts` creates and deletes a sentinel `ZZZ-E2E-*` member (does not edit existing children or settings).
  - `tests/e2e/smoke.e2e.ts` writes live settings and an existing company member’s marks, then attempts restore.
