...
-   **Offline ID Handling**: A key challenge in offline creation is handling object IDs.
    ...
    -   After the sync is successful, a final local operation is performed: the member record in IndexedDB is updated to replace the temporary ID with the real Firestore ID. This also applies to audit logs created offline.
